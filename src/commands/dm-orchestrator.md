...
allowed-tools:
  ...
  - Bash
---
# dm-orchestrator — One story, full cycle, checkpoints kept
...
1. docs/prd.md exists? Missing → STOP: "No PRD — the pipeline starts with /dm-prd <target>. Nothing to orchestrate yet."
2. docs/stories.md exists? Missing → STOP: "No stories — run /dm-stories first."
3. docs/architecture.md exists? Missing → STOP: "No architecture — run /dm-architect first."
4. docs/reviews/stories.md says `Stories ready: yes`? If missing or negative, warn (don't stop): the breakdown hasn't passed /dm-stories-review.
...
## Phase 1 — Research
If docs/research/<id>.md doesn't exist, produce it now following the dm-research contract: codebase-analysis skill on the story's scope, current state of the code, output structured by @templates/research.md. Otherwise reuse the existing file.

## Phase 2 — Design (UI stories only)
If the story has a screen and docs/designs/<id>.md doesn't exist, follow the dm-design contract: fail-closed on docs/design-system.md (missing → stop and point to /dm-design-system), ask who produces the design (agent / Claude Design / Gemini) via AskUserQuestion, output docs/designs/<id>.md + .html anchored to the design system only. A story without UI skips this phase.

## Phase 3 — Plan
If docs/plans/<id>.md doesn't exist, produce it following the dm-plan contract: small verifiable tasks, structured by @templates/plan.md.
...
## Phase 4 — Execute
Fail-closed: docs/plans/<id>.md must carry `validated: yes` in its frontmatter — missing means back to the Phase 3 checkpoint. Then delegate to the `implementer` subagent exactly as /dm-execute does, with the verified absolute worktree as its working directory: strict TDD, no branch switching, only what the plan specifies; fix mode first if a blocking review exists. Capture its summary.

## Phase 5 — Review
Delegate to the `reviewer` subagent exactly as /dm-review does: fresh context, story diff `git diff <default-branch>...feature/<id>`, test suite run by the reviewer, verdict ending with the exact `Max severity:` and `Ship allowed:` lines. Write the report to docs/reviews/<id>.md.
...
## Phase 6 — Ship
CHECKPOINT — review passed: show the verdict and ask via AskUserQuestion: "Ship now?" — options: Ship / Not now. Only an explicit Ship proceeds; then run /dm-ship's flow: mechanical gate (`grep -q '^Ship allowed: yes' docs/reviews/<id>.md`), tests on the branch, push, PR — then the project's ship strategy (manual, the default: stop at the PR; auto: merge, deploy, confirm live, clean up the branch once the merge is proven).
